...
export function CopyPrevMonthButton({ referenceMonth, prevReferenceMonth }: Props) {
  ...
  const handleCopy = () => {
    ...
    startTransition(async () => {
      const result = await copyBudgetOverridesFromPrevMonth(referenceMonth, prevReferenceMonth)
      if (result.copied === 0) {
