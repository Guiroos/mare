      ...
      if (goal.investmentTypeId) {
        ...
        const [result, last3] = await Promise.all([
          db
            ...
            .where(
              and(eq(goalContributions.userId, userId), eq(goalContributions.goalId, goal.id))
            ),
          db.query.goalContributions.findMany({
            where: and(eq(goalContributions.userId, userId), eq(goalContributions.goalId, goal.id)),
            orderBy: (c, { desc }) => [desc(c.referenceMonth)],
            limit: 3,
          }),
        ])
