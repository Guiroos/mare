...
Esta lista é gate: `__tests__/unit/docs-crypto-api.test.ts` exige que todo símbolo exportado de `lib/crypto/*.ts` apareça aqui. Export novo sem linha nesta seção deixa a suíte vermelha.

- `generateDek()` — 32 bytes aleatórios. Não é derivada de `userId` nem da MEK: uma DEK perdida é perdida, e é isso que faz o reset de conta funcionar como cripto-apagamento.
- `encryptDek(dek)` / `decryptDek(encrypted)` — embrulham a DEK com a MEK. Chamam `getMek()` **antes de qualquer I/O**, então falham cedo quando a `ENCRYPTION_MASTER_KEY` está quebrada. Usar `decryptDek` lendo `userSettings.encryptedDek` direto quando é preciso uma DEK **específica** que não pode vir memoizada (ver o gotcha do `cache()` em `getDekForUser`).
