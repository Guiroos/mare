...
- `Segment` é para conjuntos pequenos e fixos (≤4 opções previsíveis); para filtros com N dinâmico/crescente (ex: meses com histórico acumulado), usar `Select` — `Segment` usa `inline-flex` sem wrap e transborda horizontalmente em dialogs
