...
- `OpenChargesPicker` (`components/devedores/OpenChargesPicker.tsx`): componente compartilhado que renderiza a lista de cobranças em aberto com filtro de mês (`Select`, só quando >1 mês) + scroll (`max-h-48`) + checkboxes; usado por `CobrancaDialog` e `DebtPaymentDialog`. Seleção via `Set<string>` controlada pelo pai (`selectedIds`/`onSelectionChange`); `showBulkControls` opcional adiciona "Selecionar tudo/Limpar" (usar na cobrança, não no pagamento). O filtro de mês afeta só a exibição; a seleção e o total somam todas as cobranças marcadas entre meses
