# Tech Debt — Índice

Dívidas técnicas conhecidas e aceitas conscientemente. Cada entrada documenta o problema,
o motivo da decisão de não resolver agora, e o critério para revisitar.

## Itens

| # | Arquivo | Área | Severidade |
| - | ------- | ---- | ---------- |
| 1 | [acoes-multi-tabela-sem-transacao.md](./acoes-multi-tabela-sem-transacao.md) | Actions / banco | média |
| 2 | [checkbox-primitivo-ausente.md](./checkbox-primitivo-ausente.md) | Design System | baixa |
| 3 | [committed-summary-componente-no-page.md](./committed-summary-componente-no-page.md) | Design System | baixa |
| 4 | [parcelas-recalculo-ao-mudar-closing-day.md](./parcelas-recalculo-ao-mudar-closing-day.md) | Actions / parcelas | baixa |
| 5 | [copy-month-data-sem-db-transaction.md](./copy-month-data-sem-db-transaction.md) | Actions / banco | alta |
| 6 | [delete-devedor-charge-sem-transaction.md](./delete-devedor-charge-sem-transaction.md) | Actions / banco | média |
| 7 | [reorder-groups-sem-transaction.md](./reorder-groups-sem-transaction.md) | Actions / banco | baixa |
| 8 | [revalidatepath-panorama-ausente.md](./revalidatepath-panorama-ausente.md) | Cache / UX | média |
| 9 | ✅ [error-boundaries-ausentes.md](./error-boundaries-ausentes.md) | UX / resiliência | resolvida |
| 10 | [number-amount-em-vez-de-toamount.md](./number-amount-em-vez-de-toamount.md) | Convenção de código | baixa |
| 11 | [vitest-incompativel-com-vite-7.md](./vitest-incompativel-com-vite-7.md) | Tooling / testes | alta |
| 12 | [leituras-usam-neon-serverless-websocket.md](./leituras-usam-neon-serverless-websocket.md) | Performance / banco | baixa |
