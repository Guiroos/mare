...
## Testes
- Balance com ajuste negativo zera corretamente nos três laços (unit em `debtors` query).
- `createDebtPayment` com conciliação cria **duas** entradas atômicas; `totalPaid` reflete só o pagamento; validação de centavos rejeita divergência.
- `deleteDebtEntry` do pagamento remove o ajuste vinculado e reabre as cobranças.
