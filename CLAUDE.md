...
- `next/font`: `preload` é por documento, não por rota — fonte declarada no root vira `<link rel="preload">` até nas páginas que não a usam (custou 0,7 s de LCP na landing); usar `preload: false` na que não serve a rota crítica. As três famílias chegam como fonte **variável**: enxugar o array `weight` não reduz bytes, só evita peso sintetizado
